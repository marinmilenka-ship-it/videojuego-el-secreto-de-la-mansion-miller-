...
# Variables globales e interfaz
tiempo_restante = 1 * 60
intro_terminada = False
game_over_activo = False
vidas = 3
checkpoint_pos = Vec3(0, 2, -8)
posicion_inicial_enemigo = Vec3(0, 0, 5)
...
texto_vida = Text(text='Vida: 100', position=(-0.85, 0.45), origin=(-0.5, 0), scale=2, color=color.green, background=True)
texto_vida.disable()
...
class Mundo:
    def __init__(self):
        self.sala = Entity(model='sala1.glb',position=(0, 0, 0),rotation=(0, 180, 0),scale=(0.1, 0.1, 0.1),collider='mesh',double_sided=True)
        self.suelo = Entity(model='plane',scale=(500, 1, 500),position=(0, -0.05, 0),visible=False,collider='box')
        self.paredes = []
...
class Jugador(FirstPersonController):
    def __init__(self, inventario_ui, mundo_ref):
        super().__init__(position=checkpoint_pos)
        self.collider = 'box'
        self.enabled = False
    ...
    def recibir_dano(self, cantidad):
        ...
        if self.salud <= 0:
            self.salud = 0
        texto_vida.text = f'Vida: {self.salud}'
        if self.salud <= 40:
            texto_vida.color = color.red
        else:
            texto_vida.color = color.green

    def update(self):
        super().update()
        moviendose = held_keys['w'] or held_keys['a'] or held_keys['s'] or held_keys['d']
        if moviendose and self.grounded and self.enabled:
...
class Enemigo(Entity):
    def __init__(self, posicion, modelo, jugador_ref, **kwargs):
        textura = kwargs.pop('texture', None)
        super().__init__(position=posicion, collider='box', **kwargs)
        self.jugador = jugador_ref
        self.velocidad = 3.5
        self.dano = 20
        self.cooldown_ataque = 0
        self.modelo_visual = Entity(parent=self,model=modelo,texture=textura,y=1.2,rotation=(180, 180, 0))

    def update(self):
        if game_over_activo or not intro_terminada:
            return
        if self.cooldown_ataque > 0:
            self.cooldown_ataque -= time.dt
        if not self.jugador.agachado:
            dist = distance(self.position, self.jugador.position)
            pos_objetivo = Vec3(self.jugador.x, self.y, self.jugador.z)
            self.look_at(pos_objetivo)
            if dist > 0.8:
                self.position += self.forward * self.velocidad * time.dt
            if dist < 1.2 and self.cooldown_ataque <= 0:
                self.jugador.recibir_dano(self.dano)
...
class Menu:
    def __init__(self, intro_ref):
        ...
        self.fondo_menu= Entity(parent=camera.ui, model='quad',texture="fondo_original.jpg (1).jpeg",color=color.white,scale=(2,1.5))
...
def update():
    ...
    if intro_terminada and tiempo_restante > 0:
        ...
        if tiempo_restante <= 0 or protagonista.salud <= 0:
...
def click_si():
    ...
    texto_vida.text = f'Vida: {protagonista.salud}'
    texto_vida.color = color.green
    protagonista.position = checkpoint_pos
    enemigo.position = posicion_inicial_enemigo
...
enemigo = Enemigo(posicion=posicion_inicial_enemigo, modelo='enemigo.obj', texture='enemigo.png', jugador_ref=protagonista)
